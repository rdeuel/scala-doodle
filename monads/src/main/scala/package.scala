...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]()
      fs.map(_.onComplete(v => p.tryComplete(v)))
      p.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith1[S](cont: Future[T] => S): Future[S] = {
      async {
        cont(f)
      }
    }
...
/*
    def continue1[S](cont: Try[T] => S): Future[S] = {
      async {
        val v = await(f)
        try {
          cont(Success(v))
        } catch {
          case ex: Exception => cont(Failure(ex))
        } 
      }
    }
*/
